...
import { ResponsiveContainer, MobileOptimizedCard, ResponsiveGrid } from '@/components/ui/responsive-container';
...
const Shop: React.FC = () => {
  ...
  const EnhancedProductCard = React.memo(({ product }: { product: Product }) => (
    <Card className="w-full group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border-2 hover:border-primary/20 bg-card rounded-2xl overflow-hidden">
      <div className="aspect-square bg-muted relative overflow-hidden">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
          <Button size="sm" variant="secondary" className="p-2 rounded-full bg-card/90 hover:bg-card shadow-sm" onClick={() => handleQuickView(product)} aria-label="Quick View">
            <Eye className="h-4 w-4" />
          </Button>
          <Button
            size="sm"
            variant="secondary"
            className={`p-2 rounded-full bg-card/90 hover:bg-card shadow-sm ${isInWishlist(product.id) ? 'text-destructive' : ''}`}
            onClick={() => handleAddToWishlistCallback(product)}
            aria-label="Add to Wishlist"
          >
            <Heart className={`h-4 w-4 ${isInWishlist(product.id) ? 'fill-destructive' : ''}`} />
          </Button>
        </div>
        {!product.in_stock && (
        ...
        {product.rating && product.total_reviews !== undefined && (
          <div className="absolute bottom-3 left-3 bg-card/90 rounded-full px-3 py-1 text-xs font-semibold flex items-center gap-1 shadow-sm">
      ...
      <CardContent className="p-3 sm:p-4">
        <div className="space-y-3">
          <Badge variant="outline" className="text-xs text-muted-foreground border-border">{product.category}</Badge>
          <h3 className="font-semibold text-foreground line-clamp-2 group-hover:text-primary transition-colors text-base">
            {product.name}
          </h3>
          {product.vendors && (<p className="text-xs text-muted-foreground flex items-center gap-1">by <span className="font-medium text-foreground">{product.vendors.business_name}</span></p>)}
  ...
  const EnhancedProductListItem = React.memo(({ product }: { product: Product }) => (
    <Card className="w-full flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border-2 hover:border-primary/20 bg-card rounded-2xl">
      <div className="flex items-start sm:items-center flex-1 min-w-0 mb-4 sm:mb-0 sm:pr-4">
        <div className="w-24 h-24 sm:w-32 sm:h-32 flex-shrink-0 bg-muted relative overflow-hidden rounded-lg mr-4">
          {product.image_url ? (
        ...
        <div className="flex-1 space-y-1">
          ...
          {product.vendors && (<p className="text-sm text-muted-foreground flex items-center gap-1">by <span className="font-medium text-foreground">{product.vendors.business_name}</span></p>)}
          <p className="text-xl font-bold text-primary">KSh {product.price.toLocaleString()}</p>
          {product.rating && product.total_reviews !== undefined && (
  ...
  return (
    <MainLayout>
      ...
      <ResponsiveContainer className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        {/* Hero Section */}
        <div className="relative h-64 overflow-hidden bg-gradient-to-r from-primary to-primary/80 rounded-3xl mb-8 shadow-xl" style={{ backgroundImage: `linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)), url('https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=2070&q=80')`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
          <div className="relative z-10 flex items-center justify-center h-full px-6 sm:px-8 lg:px-12">
            <div className="text-center text-primary-foreground max-w-3xl mx-auto">
              <ShoppingBag className="h-16 w-16 mx-auto mb-4 text-primary-foreground/80" />
        ...
        <MobileOptimizedCard className="mb-8 shadow-lg border-border/50">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            ...
            <div className="hidden sm:block">
              <label htmlFor="category-select" className="block text-sm font-medium text-foreground mb-2">Category</label>
            ...
            {/* Sort By (Desktop) */}
            <div className="hidden sm:block">
              <label htmlFor="sort-select" className="block text-sm font-medium text-foreground mb-2">Sort By</label>
              ...
              <Sheet>
                ...
                <SheetContent side="right">
                  ...
                  <div className="py-6 space-y-6">
                    ...
                    <div>
                      <label htmlFor="mobile-category-select" className="block text-sm font-medium text-foreground mb-2 flex items-center gap-2">
                    ...
                    {/* Mobile Sort By */}
                    <div>
                      <label htmlFor="mobile-sort-select" className="block text-sm font-medium text-foreground mb-2 flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <Slider
                        ...
                        className="w-full"
                      />
                      <div className="flex justify-between items-center mt-3 text-sm font-semibold text-foreground">
        ...
        {(isLoading || isFetching) ? (
          ...
          <div className="text-center py-12">
            ...
            <Button onClick={handleClearFilters} className="bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 shadow-md flex items-center gap-2">
