...
if (searchTerm) {
query = query.or(`license_number.ilike.%${searchTerm}%,license_plate.ilike.%${searchTerm}%`);
}
...
{/* Add View and Edit buttons if needed */}
<Button variant="outline" size="sm" onClick={() => console.log('View driver:', driver.id)}>
<Eye className="h-4 w-4" />
</Button>
<Button variant="outline" size="sm" onClick={() => console.log('Edit driver:', driver.id)}>
<Edit className="h-4 w-4" />
</Button>

{/* Conditionally render Verify/Reject based on verification status */}
{driver.is_verified ? (
<Button variant="secondary" size="sm" onClick={() => handleReject(driver.id)}>
<X className="h-4 w-4 mr-1" />
Reject
</Button>
) : (
<Button variant="default" size="sm" onClick={() => handleVerify(driver.id)}>
