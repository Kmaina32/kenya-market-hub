  ...
  return useQuery<Restaurant[]>({
    ...
    queryFn: async () => {
      ...
      if (params?.minDeliveryTime && params.minDeliveryTime > 0) {
          query = query.lte('delivery_time_max', params.minDeliveryTime);
      }
      
      switch (params?.sortBy) {
        case 'rating_desc': 
          query = query.order('average_rating', { ascending: false, nullsFirst: true }); 
          break;
        case 'delivery_time_asc': 
          query = query.order('delivery_time_min', { ascending: true, nullsFirst: true }); 
          break;
        case 'min_order_asc': 
          query = query.order('min_order_value', { ascending: true, nullsFirst: true }); 
          break;
        default: 
