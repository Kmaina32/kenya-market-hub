...
(gen_random_uuid(), 'CBD Nairobi', 'JKIA Airport', ST_GeogFromText('POINT(36.8219 -1.2921)'), ST_GeogFromText('POINT(36.9278 -1.3194)'), 'taxi', 1500, 'pending', now()),
(gen_random_uuid(), 'Westlands', 'Karen', ST_GeogFromText('POINT(36.8100 -1.2676)'), ST_GeogFromText('POINT(36.7073 -1.3197)'), 'taxi', 800, 'pending', now()),
(gen_random_uuid(), 'Eastleigh', 'CBD Nairobi', ST_GeogFromText('POINT(36.8417 -1.2721)'), ST_GeogFromText('POINT(36.8219 -1.2921)'), 'matatu', 300, 'pending', now());
...
SET current_location = CASE 
    WHEN id = (SELECT id FROM public.drivers ORDER BY created_at LIMIT 1 OFFSET 0) THEN ST_GeogFromText('POINT(36.8219 -1.2921)')  -- CBD
    WHEN id = (SELECT id FROM public.drivers ORDER BY created_at LIMIT 1 OFFSET 1) THEN ST_GeogFromText('POINT(36.8100 -1.2676)')  -- Westlands
    WHEN id = (SELECT id FROM public.drivers ORDER BY created_at LIMIT 1 OFFSET 2) THEN ST_GeogFromText('POINT(36.7073 -1.3197)')  -- Karen
    WHEN id = (SELECT id FROM public.drivers ORDER BY created_at LIMIT 1 OFFSET 3) THEN ST_GeogFromText('POINT(36.8417 -1.2721)')  -- Eastleigh
    WHEN id = (SELECT id FROM public.drivers ORDER BY created_at LIMIT 1 OFFSET 4) THEN ST_GeogFromText('POINT(36.8566 -1.3103)')  -- Industrial Area
    ELSE ST_GeogFromText('POINT(36.8219 -1.2921)')  -- Default to CBD
END,
